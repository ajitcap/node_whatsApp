...
kyc_rating.get('/kyc_rating_score', (req, response) => {
    ...
    try {
        ...
        let query = "call common_skalav2.sp_kyc_score('get',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NUll,NUll,NUll)";
...
console.log("current date is ..",currentDate);
     ...
     SET @msme_no =?;SET @mgmt_quota_no =? ;SET @modified_id=?; SET @modified_name=?; SET @modified_date=? ;                                     \
     ...
     @lin_no,@cin_no,@tax_tan_no,@date_of_incorporation_no,@mobile_no_num,@email_no,@msme_no,@mgmt_quota_no,@modified_id,@modified_name,@modified_date)";
        ...
        db.query(query, ['post', data.basepoint, data.pan, data.gst, data.pf,data.esic, data.itr_current_year, data.itr_secondlast_year, data.itr_thirdlast_year
            , data.reference_contact_details1, data.reference_contact_details2, data.work_order1, data.work_order2,data.work_order_cert1,data.work_order_cert2
            , data.lin, data.cin, data.tax_tan, data.date_of_incorporation, data.mobile_no, data.email, data.msme,
            data.mgmt_quota,data.modified_id,data.modified_name,currentDate], (err, res) => {
...
kyc_rating.put('/lc_kyc_update',(req,response)=>{
    try {
        ...
        let data = req.body;
        let query="SET @apiType = ?; SET @lc_id=?; CALL common_skalav2.sp_subContractor_kyc_scroe_update(@apiType,@lc_id)";

        db.query(query,['lc_score_update',data.id],(err,req)=>{
            if(err){
                console.log("update lc--->",err);
            }
            response.status(200).json({
                status:200,
